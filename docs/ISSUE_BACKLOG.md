...
### Security Note on WebAuthn Flags (UP and UV)

**Threat Model:**
WebAuthn / passkey assertions can theoretically be generated by compromised authenticators or software stubs without active human interaction or biometric/PIN verification (omitting the User Presence `UP` and User Verification `UV` bits in `authenticatorData`). 

**What this protection addresses:**
By enforcing `RequireUserPresence(true)` and `RequireUserVerification(true)` on `NewPasskeySigner`, the library inspects the raw `authenticatorData` byte (at index 32) and fails closed if the required bits (bit 0 for UP, bit 2 for UV) are not asserted by the hardware authenticator. This prevents backends or automated signing proxies from accepting passkey assertions that were produced silently without explicit human gesture or unlock.

**What it explicitly does not do:**
This check validates the *flags reported by the authenticator in the assertion structure*, but it does not perform cryptographic WebAuthn relying-party challenge verification or origin checks itself (which are the responsibility of the smart wallet contract's `__check_auth` or the upstream WebAuthn verifier service). It relies on the authenticity of the `authenticatorData` buffer supplied to the signer.
